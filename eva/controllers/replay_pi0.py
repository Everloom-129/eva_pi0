...
TRAJ_FULL_PATH = "models/Replay/hard/brute-force/trajectory.npz"
'''
Replayer-Pi0 Mixed controller for EVA framework
Author: Jie Wang
Version: 
   2025-04-25: initiate from spacemouse_pi0 mixed controller
...
class ReplayPi0Controller:
    def __init__(self, config: ReplayConfig = ReplayConfig(), on_switch_callback=None):
        ...
        traj_full_path = TRAJ_FULL_PATH
        self.replayer_controller = Replayer(traj_full_path)
        ...
        blue_print("- Auto-switching when pineapple is detected")
    ...
    def detect_pineapple(self, image_path):
        """Run DINOX detector on image to detect pineapple"""
        # Run detection
        predictions = self.dinox_detector.get_dinox(image_path, self.config.detection_prompt)
        # Check if pineapple is detected with sufficient confidence
        for obj in predictions:
            class_name = obj.get('category', '').lower()
            confidence = obj.get('score', 0.0)
            # import ipdb; ipdb.set_trace()
            if ('pineapple' in class_name and confidence >= self.config.detection_confidence):
                blue_print(f"Pineapple detected with confidence {confidence:.2f}")
                return True
    ...
    def forward(self, observation):
        ...
        if self._state["t_step"] == 0:
            input("ReplayPi0 == Press Enter to Start...")
        ...
        if self.dinox_switch_counter % self.switch_interval == 0 and self._state["current_controller"] == 1:
            ...
            temp_pth = "./temp.jpg"
            img.save(temp_pth)
            if self.detect_pineapple(temp_pth):                   # ① interval success
